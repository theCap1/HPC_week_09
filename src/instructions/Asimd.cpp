...
uint32_t mini_jit::instructions::Asimd::dpFmadd(   uint8_t i_regGpDes,
                                                  uint8_t i_regGpMulN,
                                                  uint8_t i_regGpMulM,
                                                  uint8_t i_regGpAdd,
  ...
  l_ins |= (0x1f & i_regGpMulN) << 5;
  l_ins |= (0x1f & i_regGpAdd) << 10;
  l_ins |= (0x1f & i_regGpMulM) << 16;
  l_ins |= (0x1 & i_sPdP) << 22;
...
uint32_t mini_jit::instructions::Asimd::dpFmla(  uint8_t i_regGpDes,
                                                uint8_t i_regGpMulN,
                                                uint8_t i_regGpMulM,
                                                uint8_t i_sPdP,     // 0 = Single Precision, 1 = Double Precision
                                                uint8_t i_2o4 ){    // 0 = two values, 1 = four values (only for single precision because registers only have 128 bit)
